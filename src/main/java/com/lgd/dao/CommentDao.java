...
public class CommentDao {
    ...
    public int updateComment(Comment comment) {
        return template.update("update comment set `comment` = ? ,`email` = ?,`create_time` = ?,`nick` = ?" +
                        ",`b_id` = ? where id = ?",
                comment.getComment(),comment.getEmail(),comment.getCreate_time(),
                comment.getNick(),comment.getB_id(),comment.getId());
    }
